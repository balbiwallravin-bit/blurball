...
def inference_video(
    ...
    # Get all frames
    imgs_paths = sorted(Path(frame_dir).glob("*.jpg"))
    ...
    # ===== Save heatmaps as NPZ (per-frame, stride=4) =====
    if save_hm_npz and hm_npz_dir is not None:
        os.makedirs(hm_npz_dir, exist_ok=True)
        ...
        for img_path in result_dict.keys():
            ...
            out_path = os.path.join(hm_npz_dir, f"{stem}.npz")

            # 1) heatmap: full-res (288x512) -> stride=4 (72x128)
            hm_full = hm_results[img_path][0]["hm"]
            hm_s4 = cv2.resize(hm_full, (128, 72), interpolation=cv2.INTER_AREA).astype(np.float16)

            if first_debug:
                print("[DEBUG] hm_full:", getattr(hm_full, "shape", None), "hm_s4:", hm_s4.shape, hm_s4.dtype)
                first_debug = False

            # 2) score: detector confidence (NOT tracker)
            det_score = float("nan")
            preds_list = det_results.get(img_path, [])
            if preds_list:
                scores = []
                for p in preds_list:
                    try:
                        if isinstance(p, dict):
                            for k in ("score", "conf", "p"):
                                if k in p:
                                    scores.append(float(p[k]))
                                    break
                        ...
                        pass
                if scores:
                    det_score = max(scores)

            np.savez_compressed(out_path, hm=hm_s4, score=np.float16(det_score))
            saved += 1

        print(f"[HM] Saved {saved} heatmaps (stride=4) to {hm_npz_dir}")

        # IMPORTANT: skip visualization & drawing
    ...
    df["Frame"] = df.index
    csv_path = osp.join(str(output_dir), "traj.csv") if output_dir is not None else osp.join(frame_dir, "traj.csv")
...
class NewVideosInferenceRunner(BaseRunner):
    ...
    def _run_model(self, model=None):
        ...
        base_output = Path("/home/lht/blurtrack/video_maked")

        video_name = self._input_vid_path.stem
        date = self._input_vid_path.parent.parent.name
        hour = self._input_vid_path.parent.name

        segment_name = f"{date}__{hour}__{video_name}"
        output_dir = base_output / segment_name
        output_dir.mkdir(parents=True, exist_ok=True)

        pseudo_base = Path("/home/lht/blurtrack/pseudo/heatmaps")
        hm_npz_dir = str(pseudo_base / segment_name)
        ...
        # if self._vis_result:
        #     vis_frame_dir = str(output_dir / "frames")
        #     mkdir_if_missing(vis_frame_dir)
        # if self._vis_hm:
        #     vis_hm_dir = str(output_dir / "hm")
